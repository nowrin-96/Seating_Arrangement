...
function seed() {
  console.log('Seeding SQLite database with authentic class data...');
  const tx = db.transaction(() => {
    db.exec('DELETE FROM students; DELETE FROM benches; DELETE FROM config; DELETE FROM admin;');
    ...
    // Benches
    const benchStmt = db.prepare('INSERT INTO benches (id, name, column, gender, capacity, position) VALUES (?, ?, ?, ?, ?, ?)');
    BENCHES.forEach(b => benchStmt.run(b.id, b.name, b.column, b.gender, b.capacity, b.position));

    // Students
    const studentStmt = db.prepare(`
      INSERT INTO students (id, username, password, full_name, roll_number, gender, bench_id)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);

    GIRLS_DATA.forEach((g, idx) => {
      studentStmt.run(idx + 1, g.username, bcrypt.hashSync(g.pass, 10), g.full_name, g.roll_number, 'female', g.bench_id);
    });

    BOYS_DATA.forEach((b, idx) => {
      studentStmt.run(idx + 14, b.username, bcrypt.hashSync(b.pass, 10), b.full_name, b.roll_number, 'male', b.bench_id);
    });
  });
